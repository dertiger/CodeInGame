...
class Sample:
    ...
    def get_cost_for_player(self, player:Player) -> list:
        effective_costs = []
        for index, i in range(5):
            effective_cost = self.cost[i] - player.expertise[i]
            if effective_cost < 0:
...
while True:
    ...
    answer = ""
    player_samples = [p_samples for p_samples in samples if p_samples.carried_by == 0]
    if players[0].eta > 0:
        answer = "WAIT"
    else:
        if players[0].target == "DIAGNOSIS":
